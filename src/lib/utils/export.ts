...
export async function exportData(formatType: 'json' | 'csv') {
  ...
  if (formatType === 'json') {
    ...
    downloadFile(blob, `budget_export_${format(new Date(), 'yyyy-MM-dd')}.json`)
  } else {
...
export async function importData(file: File) {
  ...
  if (file.name.endsWith('.json')) {
    ...
    const transactions: Partial<Transaction>[] = results.data.map((row: any) => {
      const categoryId = categoryMap[row.Category?.toLowerCase()] || categories[0]?.id
      const accountId = accountMap[row.Account?.toLowerCase()] || accounts[0]?.id
