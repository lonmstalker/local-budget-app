...
export function usePlannedIncome() {
  ...
  const addIncome = useMutation({
    mutationFn: async (
      income: Omit<PlannedIncome, "id" | "createdAt" | "updatedAt">,
    ) => {
  ...
  const confirmReceived = useMutation({
    mutationFn: async ({
      ...
      const transaction: Omit<
        Transaction,
        "id" | "createdAt" | "updatedAt" | "version" | "syncStatus"
      > = {
  ...
  const calculateExpectedIncome = (days: number = 30) => {
    ...
    const probableIncome = upcoming
      ...
      .reduce(
        (sum, income) => sum + income.amount * (income.probability / 100),
        0,
      );
