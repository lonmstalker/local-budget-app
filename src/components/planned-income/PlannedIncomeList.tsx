...
export function PlannedIncomeList({ onEdit, onConfirm }: PlannedIncomeListProps) {
  const { incomes, deleteIncome, getPendingIncome } = usePlannedIncome()
  const [filter, setFilter] = useState<'all' | 'pending' | 'confirmed' | 'upcoming'>('all')
  ...
  const getIncomeStatus = (income: PlannedIncome) => {
    ...
    if (expectedDate < today) {
      return { icon: AlertCircle, color: 'text-yellow-500', label: 'Ожидается' }
    }
                ...
                <div className="flex items-start justify-between">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={cn("text-sm font-medium", getProbabilityColor(income.probability))}>
                              {income.probability}% вероятность
                            </span>
                          </div>
                          {income.probability < 100 && (
                            <Progress value={income.probability} className="w-24 h-2" />
                          )}
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      <div className="text-xl font-bold">{formatCurrencyRUB(income.amount)}</div>
                      {income.isConfirmed && income.receivedAmount && income.receivedAmount !== income.amount && (
                      ...
                      {income.probability < 100 && (
                        <div className="text-xs text-muted-foreground">
                          Ожидаемо: {formatCurrencyRUB(income.amount * income.probability / 100)}
                        </div>
                      )}
                    </div>
