...
export function FixedExpensesWidget({ currentBalance = 0 }: FixedExpensesWidgetProps) {
  const { 
    expenses, 
    calculateFreeCash, 
    getUpcomingExpenses, 
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-2xl font-bold">{formatCurrencyRUB(monthlyTotal)}</div>
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <span className="text-sm text-muted-foreground">Свободные средства</span>
            ...
            <Progress 
              value={stats.freeCash > 0 ? (stats.freeCash / currentBalance) * 100 : 0} 
              className="h-2"
            />
          </div>
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center gap-2">
              ...
              {overdueExpenses.length > 0 && (
                <Badge variant="destructive" className="text-xs">
                  {formatCurrencyRUB(overdueExpenses.reduce((sum, e) => sum + e.amount, 0))}
                </Badge>
              )}
            </div>
          </div>
            ...
            <div className="flex items-center gap-2">
              ...
              {upcomingExpenses.length > 0 && (
                <Badge variant="outline" className="text-xs">
                  {formatCurrencyRUB(upcomingExpenses.reduce((sum, e) => sum + e.amount, 0))}
                </Badge>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <Badge variant="secondary" className="text-xs">
                {formatCurrencyRUB(autopayExpenses.reduce((sum, e) => sum + e.amount, 0))}
              </Badge>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground">из {expenses.length}</span>
            </div>
          </div>
        </div>
          ...
          <div className="pt-3 border-t">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground">{nextPayment.name}</p>
              </div>
              <div className="text-right">
                <p className="text-sm font-bold">{formatCurrencyRUB(nextPayment.amount)}</p>
                <p className="text-xs text-muted-foreground">{formatDaysUntil(nextPayment.nextDueDate)}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {currentBalance > 0 && (
          <div className="pt-3 border-t">
            <div className="flex items-center justify-between text-sm">
              <span className="text-muted-foreground">Рекомендуемый резерв</span>
              <span className="font-medium">{formatCurrencyRUB(monthlyTotal * 3)}</span>
            </div>
