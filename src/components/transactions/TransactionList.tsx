...
import { useDeleteTransaction, useBulkDeleteTransactions } from '@/lib/hooks/useTransactions'
...
export function TransactionList({ transactions, onEdit, onDuplicate }: TransactionListProps) {
  ...
  const handleBulkDelete = async () => {
    ...
    if (confirm(`Вы уверены, что хотите удалить ${selectedTransactions.length} операций?`)) {
      setIsDeleting(true)
      try {
  ...
  const groupedTransactions = transactions.reduce((groups, transaction) => {
    const date = format(new Date(transaction.date), 'd MMMM yyyy', { locale: ru })
    if (!groups[date]) {
      groups[date] = []
    }
